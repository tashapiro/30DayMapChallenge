...
#get map outline of Ukraine
outline<-rnaturalearth::ne_countries(scale = 10, type = "countries", continent = NULL,
                      country = "Ukraine", geounit = NULL, sovereignty = NULL,
                      returnclass = c("sp", "sf"))
...
ggplot()+
  ...
  #title
  geom_textbox(mapping=aes(label=title, x=36, y=44.5), halign=1, box.size=NA, fill=NA,
               width = unit(4, "inch"))+
  #caption
  geom_textbox(mapping=aes(label=caption, x=23.5, y=44.1), halign=0.5, box.size=NA, fill=NA,
               width = unit(4, "inch"))+
  coord_map()+
  scale_y_continuous(limits=c(43.5,53), expand=c(0,0))+
  theme_void()+
  theme(
...
#save plot
ggsave("slava-ukraini.png", height=6, width=9, units="in")
